...
def get_high_lows_lookback(high, low, lookback_days):
    ...
    #passsing the df as a list object
...
def get_long_short(close, lookback_high, lookback_low):
    ...
    """
    #TODO: Implement function
    # creating an empty signal list object
    l_s_signal =[]
    
    # getting the sizeof the close df
    df_size = close.shape
    # getting the row and column lenght of the df
    row_len = close.shape[0]
    col_len = close.shape[1]
    
    # getting values for the columns and rows
    c_values = list(close.columns.values)
    r_values = list(close.index.values)
    
    #looping through the 2d df getting the prices for high and low then comparing them to the close price
    for i in range(0, row_len):
        for j in range(0, col_len):
            l_high = lookback_high.iloc[i][j]
            l_low = lookback_low.iloc[i][j]
            close_price = close.iloc[i][j]
            if l_low > close_price:
                l_s_signal.append(-1)
            elif l_high < close_price:
                l_s_signal.append(1)
            else:
                l_s_signal.append(0)

    # making an array from the signal and then to a df  while reshaping it with th df size
    l_s_signal_arr = np.array(l_s_signal).reshape(df_size)
    l_s_signal_df = pd.DataFrame(l_s_signal_arr,r_values, c_values)
    
    # returning the df
    return l_s_signal_df
...
def filter_signals(signal, lookahead_days):
    ...
    # getiing signal values for columns and rows
...
def get_lookahead_prices(close, lookahead_days):
    ...
    """
    #TODO: Implement function
    # creating an empty list object of lookahead prices
    l_price = []
    
    # getting the sizeof the close prices df
    df_size = close.shape
    ...
    # getting values for the columns and index
    col_values = list(close.columns.values)
    index_values = list(close.index.values)
    
    #looping through the df to acces the close price at each position from a given lookahead day
    ...
    #changing the l_prices list to df that is to be returned
    lookahead_prices = pd.DataFrame(l_price, index_values, col_values)
...
def get_return_lookahead(close, lookahead_prices):
    ...
    #TODO: Implement function
     ...
     # creating an empty list object of lookahead returns
    l_return = []
    ...
    # getting values for the columns and index in a list
    col_values = list(close.columns.values)
    index_values = list(close.index.values)
    ...
    for price in range(row_len):  
        ...
        for (cp,lp) in zip(close.iloc[price].values, lookahead_prices.iloc[price].values):
                ...
                l_ret.append(np.log(lp/cp))
        l_return.append(l_ret)
                
    # returning the loopback return list as a df
    loopahead_returns = pd.DataFrame(l_return, index_values, col_values)
    
    return loopahead_returns
...
# From My obersation, the signal returns has a normal distribution though with some distortions  which are more visble for 10 days 
# and 20 days due to outliers in the dataset.
...
def calculate_kstest(long_short_signal_returns):
    ...
    for col_name, group in long_short_signal_returns.groupby('ticker'):
        ...
        ks_value, p_value = kstest(sub,'norm', args=(long_short_signal_returns['signal_return'].mean(), long_short_signal_returns['signal_return'].std()))
...
def find_outliers(ks_values, p_values, ks_threshold, pvalue_threshold=0.05):
    ...
    # empty ist objects for outliers
    outliers = []
    
    #getting the row of series
    # loopng through the seies nad getting the the symbols that satisfy the condition
